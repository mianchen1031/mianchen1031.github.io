...
const colorCache$1 = new Map();
const bgColorCache = new Map();
let containerId$1 = null;
let schxrBg = null;
let scxjBg = null;
/**
 * 初始化桑基图
...
function colorReset$1() {
    colorCache$1.forEach((value, key) => {
        select(key).attr("fill", value);
    });
}
function clickSchxr() {
    colorReset$1();
    const svg = select(`#${containerId$1}`).select('svg');
    paths$1.schxr.forEach((id) => {
        const pathList = svg.selectAll(`path[name="${id}"]`);
        pathList.each(function () {
            if (!colorCache$1.get(this)) {
                colorCache$1.set(this, select(this).attr('fill'));
            }
        });
 ...
 * @param {*} height 
 */
async function init(id, url = "./public/关系图.svg") {
    containerId = id;
    const text$1 = await text(url);
    const app = select(`#${containerId}`);
    app.html(text$1);
    const svg = app.select("svg");
...
function click11() {
    colorReset();
    const svg = select(`#${containerId}`).select('svg');
    paths.a.forEach((id) => {
        const pathList = svg.selectAll(`circle[name="${id}"]`);
        pathList.each(function () {
            if (!colorCache.get(this)) {
                colorCache.set(this, select(this).attr('fill'));
            }
        });
...
export { init as gxInit, init$1 as sjInit };
